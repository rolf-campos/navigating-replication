...
def adjustment_costs(a, a_grid, ra, chi0, chi1, chi2, chi3):
    chi = get_Psi_and_deriv(a, a_grid, ra, chi0, chi1, chi2, chi3)[0]
    return chi


def marginal_cost_grid(a_grid, ra, chi0, chi1, chi2, chi3):
    # precompute Psi1(a', a) on grid of (a', a) for steps 3 and 5
    Psi1 = get_Psi_and_deriv(a_grid[:, np.newaxis],
                             a_grid[np.newaxis, :], ra, chi0, chi1, chi2, chi3)[1]
    return Psi1
...
def hh(Va_p, Vb_p, a_grid, b_grid, z_grid, e_grid, k_grid, beta, eis, rb, ra, chi0, chi1, chi2, chi3, Psi1):
    ...
    b_endo = (c_endo_unc + a_endo_unc + addouter(-z_grid, b_grid, -(1 + ra) * a_grid)
              + get_Psi_and_deriv(a_endo_unc, a_grid, ra, chi0, chi1, chi2, chi3)[0]) / (1 + rb)
    ...
    b_endo = (c_endo_con + a_endo_con
              ...
              + get_Psi_and_deriv(a_endo_con, a_grid, ra, chi0, chi1, chi2, chi3)[0]) / (1 + rb)
    ...
    Psi, _, Psi2 = get_Psi_and_deriv(a, a_grid, ra, chi0, chi1, chi2, chi3)
...
def get_Psi_and_deriv(ap, a, ra, chi0, chi1, chi2, chi3):
    ...
    adj_denominator = np.maximum(a_with_return, chi0)  

    core_factor = (abs_a_change / adj_denominator) ** (chi2 - 1)

    Psi_orig = chi1 / chi2 * abs_a_change * core_factor
    Psi_linear = chi3 * abs_a_change
    Psi = Psi_orig + Psi_linear
    #Psi = chi0 * abs_a_change + chi1 * (abs_a_change / adj_denominator) ** chi2 * adj_denominator
    Psi1_orig = chi1 * sign_change * core_factor
    Psi1_linear = chi3 * sign_change
    Psi1 = Psi1_orig + Psi1_linear
    Psi2_orig = -(1 + ra) * (Psi1 + (chi2 - 1) * Psi / adj_denominator)
    Psi2_linear = -(1 + ra) * chi3 * sign_change
    Psi2 = Psi2_orig + Psi2_linear
